...
#ifdef _WIN32
#define LEAF_TYPE __int64
#else
#define LEAF_TYPE long
#endif
    ...
    for (int k = 0; k < tile_dim.z(); ++k) {
      for (int j = 0; j < tile_dim.y(); ++j) {
        for (int i = 0; i < tile_dim.x(); ++i, ++voxel_count) {
          ...
          if (sparse_indexes->at(tile_index) + data_index > sparse_size) {
            printf("FICK");
          }
          /* Index computation by coordinates is reversed in VDB grids. */
          int leaf_index = compute_index(k, j, i, TILE_SIZE, TILE_SIZE);
                             ...
                             int &sparse_size)
{
  ...
  switch (grid_type) {
    case OPENVDB_GRID_BOOL:
      return image_load_preprocess<BoolGrid, unsigned LEAF_TYPE>(
                        ...
                        float *image)
{
  ...
  if (make_sparse && use_pad) {
    switch (grid_type) {
      case OPENVDB_GRID_BOOL:
        return image_load_sparse_pad<BoolGrid, unsigned LEAF_TYPE>(
    ...
    switch (grid_type) {
      case OPENVDB_GRID_BOOL:
        return image_load_sparse<BoolGrid, unsigned LEAF_TYPE>(
      ...
      case OPENVDB_GRID_FLOAT:
        return image_load_sparse<FloatGrid, float>(
            grid, sparse_indexes, resolution, min_bound, 1, image, sparse_size);
      case OPENVDB_GRID_INT32:
    ...
    switch (grid_type) {
      case OPENVDB_GRID_BOOL:
        return image_load_dense<BoolGrid, unsigned LEAF_TYPE>(grid, resolution, min_bound, 1, image);
      case OPENVDB_GRID_DOUBLE:
